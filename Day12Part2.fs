module AdventOfCode2021.Day12Part2
...
let run() =
    Console.WriteLine "Day 12 Part2"
    let inputs =
    ...
    let isTerminal cave =
        let caveName, _ = cave
        caveName = "start" || caveName = "end"

    let hasSmallDuplicates path =
        let counts =
            path
            |> List.filter (fun p ->
                let _, caveType = p
            ...
            |> List.countBy (fun p ->
                let name, _ = p
                name)
        let anyDuplicates = counts |> List.exists (fun pc ->
            let _, c = pc
            c > 1)
        anyDuplicates

    let canVisitSmallCave path cave =
        if isTerminal cave || hasSmallDuplicates path
        then not (path |> List.contains cave)
        else true
        ...
        then
            ...
            let nonDuplicatedConnections =
                ...
                |> List.filter (fun cave ->
                    let _, size = cave
                    if size = Big then true else canVisitSmallCave path cave)
