module AdventOfCode2021.Day11Part2

open System
open System.IO

let run() =
    Console.WriteLine "Day 11 Part2"
    let inputs =
    ...
    let mutable firstAllFlashStep = 0
    ...
    let simulate step =
        ...
        if (flashState |> isAllFlashes)
        then firstAllFlashStep <- step
        else ()

    let rec simulateLoop step =
        if firstAllFlashStep = 0 then
            simulate step
            simulateLoop (step + 1)
    ...
    Console.WriteLine $"Result is {firstAllFlashStep}"